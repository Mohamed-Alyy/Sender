...
internal final class GalleryPickerHelper: NSObject, VideoPickerDelegate {
    ...
    internal func pickCamera(in screen: UIViewController?, type: PickingType = .picture) {
        ...
        if let pick = self.picker {
            screen?.present(pick, animated: true)
        }
    }
    internal func pickGallery(in screen: UIViewController?, type: PickingType = .picture) {
