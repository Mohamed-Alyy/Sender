...
private var maxCharTextfields: [UITextField: Int] = [:]
extension UITextField {
    /// SwifterSwift: Border width of view; also inspectable from Storyboard.
    @IBInspectable public var maxChar: Int {
        get {
            return self.maxChar
        }
        set {
            if newValue > 0 {
                self.delegate = self
                maxCharTextfields[self] = newValue
            }
        }
    }
...
extension UITextField {
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        let maxLength = maxCharTextfields[textField] ?? 1
